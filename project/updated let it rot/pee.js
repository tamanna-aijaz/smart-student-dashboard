...
function rollExists(roll, ignoreIndex = null) {
  const records = loadRecords();
  return records.some((s, idx) => {
    if (!s.roll) return false;
...
function showFailureOverlay(targetPanel, message) {
  const overlayDiv = document.createElement("div");
  overlayDiv.className = "success-overlay";
  overlayDiv.innerHTML = `
    <span style="font-size:3.5rem;color:#ff4d4d;">✖</span>
...
function onSubmit(e) {
  ...
  const rollValue = document.getElementById("rollNumber").value.trim();

  if (rollExists(rollValue)) {
    showFailureOverlay(formPanel, "Roll number already exists");
    return;
  }
...
if (searchForm) {
  searchForm.addEventListener("submit", (e) => {
    ...
    const match = records.find(
      (s) => s.roll && s.roll.toLowerCase() === query
    );
    if (match) {
    ...
    if (matchIndex >= 0) {
      ...
      if (editBtn) {
        editBtn.addEventListener("click", () => {
         ...
         const newRoll = document.getElementById("rollNumber").value.trim().toLowerCase();
         const oldRoll = latest[matchIndex].roll.toLowerCase();

         // 🔴 CASE 1: Roll number changed → check duplicate
         if (newRoll !== oldRoll) {
          const rollExists = latest.some(
         (s, i) => s.roll.toLowerCase() === newRoll && i !== matchIndex
          );
         ...
         // ✅ CASE 2: Safe to update → REPLACE record (not add)
         latest[matchIndex] = {
         name: document.getElementById("fullName").value.trim(),
         roll: document.getElementById("rollNumber").value.trim(),
         semester: document.getElementById("semester").value,
...
function showRecordsCapsule() {
  ...
  document.querySelectorAll(".editBtn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const updatedRoll = document.getElementById("rollNumber").value.trim();

       if (rollExists(updatedRoll, idx)) {
       showFailureOverlay(formPanel, "Roll number already exists");
       return;
       }
...
document.addEventListener("click", (e) => {
  if (e.target.classList.contains("close-btn")) {
    showHome();   // cancel current panel and go home
  }
});
